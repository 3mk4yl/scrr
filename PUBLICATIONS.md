# Publications Index

This document indexes all published volumes of the *Strategic Cyber Risk Report*. Follow the links below to purchase the formatted narrative reports or to access the raw, open-source research dossiers associated with each book.

---
## Volume 1 (2026)

### Report No. 1: Adversarial Machines
**Subtitle:** *The Convergence of Autonomous Offense, Poisoned Intelligence, and the Collapse of Digital Trust*

**Release Date:** March 2026

> **Synopsis:** The old rules are dead. As autonomous agents execute multi-stage ransomware and LLMs discover zero-days in minutes, the operational asymmetry between machine-time attacks and human-time defense has become the defining characteristic of the modern threat landscape. This report maps the acceleration of AI-driven cyber warfare.

* 🛒 **[Purchase the Book on Amazon KDP (Amazon Kindle)](https://amzn.eu/d/06HDzURR)**
* 📁 **[Access the Raw Research Dossiers (GitHub Folder)](dossiers/2026/01-adversarial-machines)**

**Referenced Dossiers in this Volume:**
* `[1]` [Autonomous Offensive Cyber Operations & AI-Driven Malware](dossiers/2026/01-adversarial-machines/1_autonomous_offensive_cyber_operations.md)
* `[2]` [Data Poisoning and LLM Manipulation](dossiers/2026/01-adversarial-machines/2_adversarial_attacks_on_enterprise_llm.md)
* `[3]` [The Evolution of Agentic Ransomware](dossiers/2026/01-adversarial-machines/3_weaponization_of_ai_for_advanced_social_engineering.md)
* `[4]` [Automated Command & Control Infrastructure](dossiers/2026/01-adversarial-machines/4_machine_learning_model_poisoning.md)
* `[5]` [Defensive Countermeasures & Hardware Telemetry](dossiers/2026/01-adversarial-machines/5_adversarial_machine_learning_for_defensive_evasion.md)

---
### Report No. 2: *Upcoming*
**Subtitle:** *Upcoming*

**Release Date:** *tba*

> **Synopsis:** *Upcoming*

* 🛒 **[Link coming soon]()**
* 📁 **[Folder coming soon]()**

